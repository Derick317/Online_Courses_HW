...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        # obs_tensor = torch.tensor(observation, dtype=torch.float32, device=ptu.device)
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss = - torch.mean(distributions.log_prob(actions) * advantages)
        loss.backward()
        self.optimizer.step()
        ...
        if self.nn_baseline:
            ...
            # TODO
            q_normalized = ptu.from_numpy((q_values - np.mean(q_values)) / np.std(q_values))
            self.baseline_optimizer.zero_grad()
            baseline_loss = self.baseline_loss(advantages, q_normalized)
            baseline_loss.requires_grad_(True)
